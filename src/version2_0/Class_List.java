package version2_0;
/* File: Class_List.java
 * Written by: Jacob Schwell
 * This is the table where the user adds course information to be plotted in the visual schedule
 * 
 * Update 11/10/2014
 * 	- Set the visual schedule frame to not resizable
 * 
 * 
 * next update: count characters in import to make courses 22 characters long, then export with 1 tab
 */
...
//			course.get(r).setColumns(22);
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 if (cmd.equals(addRow.getActionCommand())){
			 ...
			 visible.add(new Checkbox("visible", true));
				course.add(new JTextField(""));
//				course.get(course.size()-1).setColumns(22);
				sTime.add(new JTextField(""));
				eTime.add(new JTextField(""));
				days.add(new JTextField(""));
				this.removeAll();
...
//					
		 ...
		 if (cmd.equals(create.getActionCommand())){
			 ...
			 f2.setSize(855, 17*Course.SCALE+25);
			 f2.setResizable(false);
//			 f2.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
			 f2.setVisible(true);
			 w2.add(new ScheduleVisual(courseList, firstLoad));
		 ...
		 if (cmd.equals("analyze")){
			 ...
			 String ncourse, nstart, nend, ndays,nvis;
			 s.useDelimiter("\t");
			 while (s.hasNext()){
				 ncourse = s.next();
				 nstart = s.next();
...
//				 visible.add(new Checkbox("visible", true));
					...
					if (nvis.toLowerCase().contains("true"))
						visible.add(new Checkbox("visible", true));
					else
						visible.add(new Checkbox("visible", false));
					this.removeAll();
					this.setLayout(new GridLayout(rows+1, 5));
					for(int r = 0; r < rows; r++){
						this.add(visible.get(r));
